 ...
 * Copied and changed by jixiaoyong1995@gmail.com on 2019/04/21 17:18:30
 */
...
public class AppOpener {
    ...
    public static void openInCustomTabsOrBrowser(@NonNull Context context, @NonNull String url) {
        if (isBlank(url)) {
            Toast.makeText(context, context.getString(R.string.invalid_url), Toast.LENGTH_LONG).show();
            return;
        }
        //check http prefix
        if (!url.contains("//")) {
    ...
    public static void openInBrowser(@NonNull Context context, @NonNull String url) {
        ...
        if (intent != null) {
            ...
            Toast.makeText(context, context.getString(R.string.no_browser_clients), Toast.LENGTH_LONG).show();
        }
    }
